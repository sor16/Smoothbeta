Densevalm22 <- function(th,RC){
    ...
    R_Beta=(1+sqrt(5)*RC$dist/exp(phi_b)+5*RC$dist^2/(3*exp(phi_b)^2))*exp(-sqrt(5)*RC$dist/exp(phi_b))
    Sig_x=rbind(cbind(RC$Sig_ab,RC$m1),cbind(RC$m2,exp(sig_b2)*R_Beta))
    
    X=rbind(cbind(1,l,diag(l)%*%RC$A),RC$Z)
    L=t(chol(trick*(X%*%Sig_x%*%t(X)+Sig_eps)))/sqrt(trick)
    ...
    W=solve(L,X%*%Sig_x)
    x_u=RC$mu_x+t((1/sqrt(trick))*chol(trick*Sig_x))%*%rnorm(RC$n+2)
